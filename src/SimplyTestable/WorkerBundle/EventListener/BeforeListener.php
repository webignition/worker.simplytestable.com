...
namespace SimplyTestable\WorkerBundle\EventListener;
            ...
            foreach ($controller->getInputDefinition($methodName)->getArguments() as $inputArgument) {
                ...
                if ($inputArgument->isRequired()) {
                    if (!$methodArguments->has($inputArgument->getName())) {
                        $this->logger->warning('BeforeListener' . get_class($controller) . '::' . $methodName.': missing required argument ['.$inputArgument->getName().']');
